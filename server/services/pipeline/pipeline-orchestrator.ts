...
export class PipelineOrchestrator {
  ...
  private async _processWithinTransaction(tx: typeof db): Promise<number> {
    ...
    for (const event of unprocessedEvents) {
      ...
      if (automationLevel === 'MANUAL') {
        try {
          ...
          await notificationHub.emit('manual_pipeline_event', {
            eventId: event.id, eventType: event.eventType, entityId: event.entityId,
          });
        } catch (err) {
